...
- `node/stats/disk-*-node.stats` — Disk I/O (IOPS and throughput)
- `node/stats/net-rcv-node.stats`, `net-xmt-node.stats` — Network in MiB/s
- `etcd/stats/db-size.stats` — etcd database size in GiB
...
- `resource/stats/pvc-usage.stats` — PVC consumption in GiB
...
- **etcd DB size:** Default quota is 8 GiB. Flag if observed or projected size
  approaches this limit.
- **Max pods:** Default is 250 per node (110 on some configurations). Report
  non-terminated pod count as a percentage of the limit.
