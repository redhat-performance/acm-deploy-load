...
| `node/stats/disk-*-node.stats` | Disk I/O | IOPS or MB/s | P95 for disk sizing |
| `node/stats/nonterm-pods-node.stats` | Non-terminated pods | count | Max vs max-pods limit |

### etcd (always present on control plane)
| File | Metric | Unit | Sizing Use |
|------|--------|------|------------|
| `etcd/stats/db-size.stats` | DB total size | GiB | P95 vs 8 GiB quota |
| `etcd/stats/db-size-in-use.stats` | DB used size | GiB | Actual data size |
...
| `resource/stats/pods.stats` | Total pod count (incl. terminated) | count |
| `resource/stats/configmaps.stats` | ConfigMap count | count |
| `resource/stats/pvc-usage.stats` | PVC usage per namespace | GiB |
